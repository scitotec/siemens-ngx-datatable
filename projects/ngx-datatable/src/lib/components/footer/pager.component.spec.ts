  ...
  describe('localisation', () => {
    ...
    describe('takes messages-overrides from table', () => {
      ...
      it('first button', () => {
        ...
        expect(firstButton.attributes['aria-label']).toEqual('link: first page');
      });

      it('previous button', () => {
        setMessages({ ariaPreviousPageMessage: 'link: previous page' })
        expect(previousButton.attributes['aria-label']).toEqual('link: previous page');
      });

      it('next button', () => {
        setMessages({ ariaNextPageMessage: 'link: next page' })
        expect(nextButton.attributes['aria-label']).toEqual('link: next page');
      });

      it('last button', () => {
        setMessages({ ariaLastPageMessage: 'link: last page' })
        expect(lastButton.attributes['aria-label']).toEqual('link: last page');
      });
